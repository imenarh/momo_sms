...
1. Start MySQL and install the `mysql` client.
2. Create tables and load sample data:

```bash
mysql -u root -p < database/database_setup.sql
mysql -u root -p momo_sms_db < database/insert_test_data.sql
```

1. Run demo queries: `mysql -u root -p momo_sms_db < database/crud_tests.sql`

Scripts: `database/database_setup.sql` (DDL), `database/insert_test_data.sql` (seed), `database/crud_tests.sql` (CRUD demos).
...
- Database Design Documentation [(PDF)](docs/Database%20Design%20Document.pdf). 
- Sample query terminal output [crud output](docs/crud_output.txt)
