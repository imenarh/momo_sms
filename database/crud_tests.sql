...
SELECT 'All transactions with category name' AS '';
...
SELECT 'Total amount moved per category' AS '';
...
SELECT 'All parties involved in transaction 6 (transfer)' AS '';
SELECT u.full_name, u.user_type, p.role
FROM transaction_participants p
JOIN users u ON p.user_id = u.id
WHERE p.transaction_id = 6;

SELECT 'Every transaction Jean Pierre Uwimana sent' AS '';
SELECT t.id, t.reference_id, t.amount, t.transaction_date
FROM transactions t
JOIN transaction_participants p ON t.id = p.transaction_id
JOIN users u ON p.user_id = u.id
WHERE u.full_name = 'Jean Pierre Uwimana' AND p.role = 'SENDER';

SELECT 'INSERT a new withdrawal transaction' AS '';
...
VALUES
    ('99999999999', 6, 5000.00, 20280.00, 100.00, 'COMPLETED', '2024-05-13 09:15:00',
     'You have withdrawn 5000 RWF from MoMo Agent Remera 36521838. Fee was 100 RWF. New balance: 20280 RWF.');
...
SELECT 'UPDATE a transaction status (flagged as REVERSED)' AS '';
UPDATE transactions SET status = 'REVERSED' WHERE id = @new_txn_id;
SELECT id, status FROM transactions WHERE id = @new_txn_id;

SELECT 'UPDATE a user phone number' AS '';
UPDATE users SET phone_number = '250788999888' WHERE full_name = 'Marie Claire Mukamana';
SELECT id, full_name, phone_number FROM users WHERE full_name = 'Marie Claire Mukamana';

SELECT 'DELETE a transaction (CASCADE into transaction_participants)' AS '';
